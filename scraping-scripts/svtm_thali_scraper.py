...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('svtm_thali.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class SVTMThaliScraper:
    """
    Complete scraper for SVTM Jewels Thali/Maangalyam Collection
    Extracts images + full metadata (description, price, product details)
    """
    
    SITE_BASE = "https://svtmjewels.com"
    COLLECTION_URL = f"{SITE_BASE}/collections/thaali-maangalyam"
    
    def __init__(self, output_dir="thali_data"):
    ...
    def scrape_collection(self):
        """Scrape all products from the thali-maangalyam collection."""
        logger.info("="*60)
        logger.info("SVTM Jewels - Thali/Maangalyam Collection Scraper")
        logger.info("Target: 8 products with complete metadata")
        logger.info("="*60)
    ...
    def run(self):
        ...
        self.save_data(full_catalog, "svtm_thali_collection.json")
        ...
        logger.info(f"Output: thali_data/svtm_thali_collection.json")
        logger.info("="*60)
